...
% author: C. David Remy, Iskandar Khemakhem, IAMS, Uni Stuttgart, 2025
    ...
    grid_min  = [config.cont.gridOpCondMin, config.cont.gridParamMin];
    grid_max  = [config.cont.gridOpCondMax, config.cont.gridParamMax];
    grid_step = [config.cont.gridOpCondStep, config.cont.gridParamStep];
    grid_min  = grid_min(:).';
    grid_max  = grid_max(:).';
    grid_step = grid_step(:).';
