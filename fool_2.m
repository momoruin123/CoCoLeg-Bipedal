...
ub_state = [0;  0.5*pi;  0.5*pi; -0.01; -0.01];
lb_state = [0; -0.5*pi; -0.5*pi; -inf; -inf];
...
% mid_k = floor(N/2) + 1;  % 或者 ceil(N/2)
% q_k = opt_vars(idx_q(mid_k));
% SwingFoot = cons_SwingFoot_m(q_k, p);
% constraints_eq = vertcat(constraints_eq, SwingFoot(2) - 0.01);
...
for k = start_k:end_k
    q_k = opt_vars(idx_q(k));
    SwingFoot = cons_SwingFoot_m(q_k, p);
    constraints_ineq = vertcat(constraints_ineq, SwingFoot(2) - 0.01);
end
...
% ===== 第一阶段：宽松精度 =====
opts1 = struct();
opts1.ipopt.print_level = 5;          % 日志输出级别
opts1.ipopt.tol = 1e-3;               % 主收敛容差（宽松）
opts1.ipopt.constr_viol_tol = 1e-3;   % 约束违反容忍度（宽松）
opts1.ipopt.max_iter = 1000;          % 最大迭代次数
opts1.ipopt.acceptable_tol = 1e-2;    % 容许精度
...
% ===== 第二阶段：高精度 =====
opts2 = struct();
opts2.ipopt.print_level = 5;          % 日志输出级别
opts2.ipopt.tol = 1e-8;               % 主收敛容差（严格）
opts2.ipopt.constr_viol_tol = 1e-8;   % 约束违反容忍度（严格）
opts2.ipopt.max_iter = 2000;          % 最大迭代次数
opts2.ipopt.acceptable_tol = 1e-6;    % 容许精度
